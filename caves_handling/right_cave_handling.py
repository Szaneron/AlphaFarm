...
first_checkpoint = ['First checkpoint', resource_path('templates/right_cave/exit_checkpoint.PNG')]
second_checkpoint = ['Second checkpoint', resource_path('templates/right_cave/second_checkpoint.PNG')]
third_checkpoint = ['Third checkpoint', resource_path('templates/right_cave/third_checkpoint.PNG')]
fourth_checkpoint = ['Fourth checkpoint', resource_path('templates/right_cave/fourth_checkpoint.PNG')]
fifth_checkpoint = ['Fifth checkpoint', resource_path('templates/right_cave/fifth_checkpoint.PNG')]
boss_checkpoint = ['Boss checkpoint', resource_path('templates/right_cave/boss_checkpoint.PNG')]
exit_checkpoint = ['Exit checkpoint', resource_path('templates/right_cave/exit_checkpoint.PNG')]


def right_cave_path():
    """
    Navigates the character through a predefined path in the right cave, encountering checkpoints and handling combats.
    ...
    for checkpoint in checkpoint_list:
        while True:
            if checkpoint == 'first checkpoint':
                move_to_checkpoint(first_checkpoint[0], first_checkpoint[1], 0.7, 180, 200)
            elif checkpoint == 'second checkpoint':
                move_to_checkpoint(second_checkpoint[0], second_checkpoint[1], 0.7, 0, 70)
            elif checkpoint == 'third checkpoint':
                move_to_checkpoint(third_checkpoint[0], third_checkpoint[1], 0.7, 200, 150)
...
def back_to_cave_enter_from_boss(fifth_checkpoint, fourth_checkpoint, third_checkpoint, second_checkpoint,
    ...
    Parameters:
    - fifth_checkpoint (List): Coordinates and template path for the fifth checkpoint.
    - fourth_checkpoint (List): Coordinates and template path for the fourth checkpoint.
